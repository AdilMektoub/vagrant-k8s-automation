...
```
$ git clone https://rndwww.nce.amadeus.net/git/scm/dmmfdv/cgc-workplace.git
$ cd cgc-workplace/Vagrant-Kubernetes
$ tree
.
|-- READ.ME
|-- Vagrantfile
...
* We also assigned the to ingress controller services external IP the master node IP.
 ```
...
ingress-nginx-controller-admission   ClusterIP   10.99.197.151    <none>          443/TCP                      15h
 ```
Set a couple of variables we will use later during the test
```
export NGINX_ICIP=192.168.56.10
export NGINX_ICPORT=443
```
...
```
# Target banana application pod
curl https://fruits.singleton.net:$NGINX_ICPORT/banana --resolve fruits.singleton.net:$NGINX_ICPORT:$NGINX_ICIP --insecure
banana

# Target apple application pod
curl https://fruits.singleton.net:$NGINX_ICPORT/apple --resolve fruits.singleton.net:$NGINX_ICPORT:$NGINX_ICIP --insecure
apple
```

**Explanation**
* As described by the fruits-ingress we expose externally to the k8s cluster our application via the FQDN fruits.singleton.net on the port 5678
* To access our application we should then simply target the FQDN:PORT using curl like:
    * curl https://fruits.singleton.net:5678/banana
or
    * curl https://fruits.singleton.net:5678/apple
* In order to be able to do so we first need firs to change the system file /etc/hosts and add our FQDN assiciated to the IP address of
the ingress controlle, which in our particular case we have configured to be the same as our k-master node (192.168.56.10)
    ***/etc/hosts***
    ```
    ...
    ````
    this will definitively work but here we have used the ***curl --resolve*** functionality for FQDN resolution.
    ```
    curl--resolve fruits.singleton.net:$NGINX_ICPORT:$NGINX_ICIP https://fruits.singleton.net:$NGINX_ICPORT/apple --insecure
    ```
    This force ***curl*** to use *$NGINX_ICIP* as the IP address when requesting "fruits.singleton.net " over port *$NGINX_ICPORT*,
    remember we previously set those variables before.
